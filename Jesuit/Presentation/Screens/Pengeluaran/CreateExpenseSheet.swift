//
//  CreateExpenseSheet.swift
//  Jesuit
//
//  "Pengeluaran Kas Baru" — create-expense form presented from the Pengeluaran
...
//  (POST /cash-transactions/submit), transaction_type=disbursement.
//
...
struct CreateExpenseSheet: View {
    ...
    private struct LineEdit: Identifiable {
        let id = UUID()
        let draft: PengeluaranPresenter.LineDraft
        let original: PengeluaranPresenter.LineDraft?
    }
...
private struct EditLineSheet: View {
    @State var draft: PengeluaranPresenter.LineDraft
    let form: PengeluaranPresenter.CreateForm
    let onCommit: () -> Void
                ...
                digits: Binding(
                    ...
                    set: { newDigits in
                        ...
                        draft.foreignAmountText = (form.exchangeRate > 0 && rp > 0)
                            ? PengeluaranPresenter.CreateForm.foreignString(rp / form.exchangeRate)
                            : ""
                    }
                )
            )
