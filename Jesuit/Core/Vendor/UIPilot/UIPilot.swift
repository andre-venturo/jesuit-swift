//
//  Vendored from github.com/canopas/UIPilot (MIT), version 2.0.2.
//  Patched: PopAwareUINavigationController re-enables the interactive
//  edge-swipe-back gesture (UIKit disables it when the nav bar is hidden).
//
...
public class UIPilot<T: Equatable>: ObservableObject {
    ...
    private let logger: UIPilotLogger
    ...
    /// App-settable veto for the interactive edge-swipe-back gesture. Return false to
    /// block the swipe-pop for the current top route (e.g. the home root, which handles
    /// the swipe itself with an exit prompt instead of popping). nil = allow (stack > 1).
    public var interactivePopGate: (() -> Bool)?
...
struct NavigationControllerHost<T: Equatable, Screen: View>: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UINavigationController {
        ...
        navigation.popGate = { [weak uipilot] in
            uipilot?.interactivePopGate?() ?? true
        }
...
class PopAwareUINavigationController: UINavigationController, UINavigationControllerDelegate, UIGestureRecognizerDelegate
{
    var popHandler: (() -> Void)?
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.delegate = self
        // UIKit kills edge-swipe-back whenever the nav bar is hidden (UIPilot hides
        // its bar for screens that bring their own NavigationStack). Re-arm it; the
        // count>1 guard stops a root-swipe deadlock.
        interactivePopGestureRecognizer?.delegate = self
    }

    func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        viewControllers.count > 1 && (popGate?() ?? true)
    }
